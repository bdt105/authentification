...
class Prescription {
    ...
    deleteFormIdCallback(err, rows, callback, id) {
        if (err) {
            ...
            let prescription = new databaseObjects_2.DatabaseTable(this.token, this.tableName, this.idFieldName);
            prescription.deleteFromId((err, rows) => callback(err, rows), "idprescription", id.toString());
        }
    }
    deleteFromId(callback, id) {
        if (id) {
            let prescriptionline = new databaseObjects_2.DatabaseTable(this.token, "prescriptionline", "idprescriptionline");
    ...
    save(callback, id, body) {
        if (id) {
            let prescriptions = new databaseObjects_2.DatabaseTable(this.token, this.tableName, this.idFieldName);
